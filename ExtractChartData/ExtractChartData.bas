...
Dim i As Long
    ...
    For Each TargetSeries In TargetChart.FullSeriesCollection
        'データセル範囲が連続している場合のみ処理
        If Len(TargetSeries.FormulaLocal) - Len(Replace(TargetSeries.FormulaLocal, ",", "")) <> 3 Then
            MsgBox "グラフの指定データセル範囲が連続していないため、抽出できません。"
            Exit Sub
        End If
        
        If ExtractSheet Is Nothing Then
            ...
            Set ExtractChart = ExtractChart.Location(Where:=xlLocationAsObject, Name:=ExtractSheet.Name)
        End If
        ...
        If TargetLabelStr <> "" And InStr(TargetLabelStr, "$") > 0 Then
        ...
        With ExtractSheet
            ...
            If TargetLabelStr <> "" Then
                ...
                If InStr(TargetLabelStr, "$") > 0 Then
                ...
                Else
                    ExtractLabelRange.Value = Replace(TargetLabelStr, """", "")
                End If
    ...
    'タイトルが数式の場合、数式で転記
    Set ExtractFormulaRange = ExtractSheet.Cells(2, Columns.Count).End(xlToLeft).Offset(-1, 0)
